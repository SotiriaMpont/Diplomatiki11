...
def Find_Best_Distributors_of_the_day_Rating_Costumer():
    ...
    if len(result) == 0: # elegxos. Den uparxei shift me auth thn hmerominia 
        ...
        sql1 = "SELECT id_rating_costumer, AVG(criterion1), AVG(criterion2), AVG(criterion3), AVG((criterion1+criterion2+criterion3)/3) as avg_rating FROM RatingFromCostumer JOIN Distributor ON RatingFromCostumer.id_rating_costumer = Distributor.id_distributor JOIN Shift ON RatingFromCostumer.dat_shif_costumer = Shift.date_shift WHERE Shift.date_shift = %s GROUP BY id_rating_costumer"
        val1 = (hmeromhnia,)
        cursor.execute(sql1, val1)
        results = cursor.fetchall()
        # kanw tous distributors ranking basi tou mesou orou tous kai tous apothikeuw stin basi
        ranked_results = sorted(results, key=lambda x: x[4], reverse=True)[:10]
        for i, result in enumerate(ranked_results):
            distributor_id = result[0]
            avg_rating = result[4]
            print(f"{i+1}. Distributor {distributor_id} has an average rating of {avg_rating:.2f}")
        ...
        mydatabase.commit()
        print("Ranking based on the average of the rating the distributors got from costumers that day")
